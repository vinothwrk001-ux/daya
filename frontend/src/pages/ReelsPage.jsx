...
export function ReelsPage() {
  ...
  async function handleLike(reel) {
    ...
    try {
      ...
      setReels((current) =>
        current.map((item) =>
          item._id === reel._id ? { ...item, liked: result.liked, likesCount: result.likesCount } : item
        )
      );
    } catch (error) {
  ...
  async function handleSave(reel) {
    ...
    try {
      ...
      setReels((current) =>
        current.map((item) =>
          item._id === reel._id ? { ...item, saved: result.saved, savesCount: result.savesCount } : item
        )
      );
    } catch (error) {
